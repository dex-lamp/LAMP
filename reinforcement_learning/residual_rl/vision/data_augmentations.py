...
def batched_random_crop(img, rng, *, padding, num_batch_dims: int = 1):
    ...
    rng, rng_crop, rng_color, rng_contrast = jax.random.split(rng, 4)

    # Flatten batch dims
    original_shape = img.shape
    img = jnp.reshape(img, (-1, *img.shape[num_batch_dims:]))

    rngs = jax.random.split(rng_crop, img.shape[0])
    ...
    # Add random color jittering
    # Random brightness
    # delta = jax.random.uniform(rng_color, (), minval=-0.2, maxval=0.2)
    # img = adjust_brightness(img, delta)

    # # Random contrast
    # factor = jax.random.uniform(rng_contrast, (), minval=0.8, maxval=1.2)
    # mean = jnp.mean(img, axis=(-2, -1), keepdims=True)
    # img = jnp.clip(factor * (img - mean) + mean, 0.0, 1.0)
...
def rgb_to_hsv(r, g, b):
    ...
    Matches the standard RGB saturation adjustment formula.
